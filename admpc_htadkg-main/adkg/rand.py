...
class Rand_Pre(Rand):
    ...
    async def acss_step(self, values):
        ...
        # 这里的话应该是 n-parallel ACSS，看一下怎么做到的
        # 在上一层中只需要创建一次avss即可
        self.acss_tasks[0] = asyncio.create_task(self.acss.avss(0, values=values))
        # for i in range(self.n):
        #     if i == self.my_id:
        #         self.acss_tasks[i] = asyncio.create_task(self.acss.avss(0, values=values))
        #     else:
...
class Rand_Foll(Rand):
    ...
    async def run_rand(self, w, rounds):
        acss_signal = asyncio.Event()
        self.acss_task = asyncio.create_task(self.acss_step(acss_signal))

        # await acss_signal用来确保收到足够多的msgs
        await acss_signal.wait()
        acss_signal.clear()
    ...
    async def acss_step(self, acss_signal):
        # 这里 ADKGMsgType.ACSS 都是 acss 有可能会和接下来的 trans 协议冲突
        acsstag = RANDMsgType.ACSS
        acsssend, acssrecv = self.get_send(acsstag), self.subscribe_recv(acsstag)
        ...
        for i in range(self.n):
                self.acss_tasks[i] = asyncio.create_task(self.acss.avss(0, i))
        ...
        while True:
            ...
            if len(outputs) >= self.n - self.t:
                # print("Player " + str(self.my_id) + " Got shares from: " + str([output for output in outputs]))
                # outputs 存在MPC实例中的Future
                acss_signal.set()

            if len(outputs) == self.n:
                self.rand_instance.acss_outputs = outputs
                return 
